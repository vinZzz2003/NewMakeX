...
class _ChampionshipScheduleState extends State<ChampionshipSchedule> {
  ...
  Future<List<Map<String, dynamic>>> _getMatchesFromBracket() async {
    ...
    // Determine which bracket table to use
    ...
    // Get all series from the bracket table with PROPER ORDERING
    final result = await conn.execute("""
      SELECT 
        match_id,
        round_name,
    ...
    // Separate matches by bracket side for proper time calculation
    final List<Map<String, dynamic>> winnersSeries = [];
    final List<Map<String, dynamic>> losersSeries = [];
    final List<Map<String, dynamic>> grandSeries = [];
    
    for (final row in result.rows) {
      final data = row.assoc();
      final side = data['bracket_side']?.toString() ?? 'winners';
      
      if (side == 'winners') {
        winnersSeries.add(data);
      } else if (side == 'losers') {
        losersSeries.add(data);
      } else {
        grandSeries.add(data);
      }
    }
    
    // Get settings values
    final int matchesPerSeries = _settings?.matchesPerAlliance ?? 3;
    final int durationMinutes = _settings?.durationMinutes ?? 6;
    final int intervalMinutes = _settings?.intervalMinutes ?? 0;
    final int timePerMatch = durationMinutes + intervalMinutes;
    
    // Start time from settings
    int currentHour = _settings?.startTime.hour ?? 13;
    int currentMinute = _settings?.startTime.minute ?? 0;
    int timeSlotIndex = 0;
    ...
    // Helper function to process a series
    Future<void> processSeries(Map<String, dynamic> seriesData, String seriesType) async {
      final alliance1Id = int.tryParse(seriesData['alliance1_id']?.toString() ?? '0') ?? 0;
      final alliance2Id = int.tryParse(seriesData['alliance2_id']?.toString() ?? '0') ?? 0;
      final roundNumber = int.tryParse(seriesData['round_number']?.toString() ?? '1') ?? 1;
      final matchPosition = int.tryParse(seriesData['match_position']?.toString() ?? '1') ?? 1;
      final bracketSide = seriesData['bracket_side']?.toString() ?? 'winners';
      
      // Skip if no teams (TBD)
      if (alliance1Id == 0 && alliance2Id == 0) {
        print("  ⏭️ Skipping $seriesType Round $roundNumber Match $matchPosition: No teams");
        return;
      }
      
      // Get alliance details
      Map<String, dynamic>? alliance1;
      Map<String, dynamic>? alliance2;
      
      if (alliance1Id > 0) {
        alliance1 = await _getAllianceById(alliance1Id);
      }
      if (alliance2Id > 0) {
        alliance2 = await _getAllianceById(alliance2Id);
      }
      
      // Determine if this is a break match (series already decided)
      bool isSeriesDecided = false;
      int? seriesWinnerId;
      
      try {
        // Check if series is already decided (2 wins for one alliance)
        final seriesCheck = await conn.execute("""
          SELECT 
            COUNT(CASE WHEN winner_alliance_id = :a1 AND is_completed = 1 THEN 1 END) as wins_a1,
            COUNT(CASE WHEN winner_alliance_id = :a2 AND is_completed = 1 THEN 1 END) as wins_a2
        ...
        if (seriesCheck.rows.isNotEmpty) {
          final check = seriesCheck.rows.first.assoc();
          int winsA1 = int.tryParse(check['wins_a1']?.toString() ?? '0') ?? 0;
          int winsA2 = int.tryParse(check['wins_a2']?.toString() ?? '0') ?? 0;
          
          if (winsA1 >= 2) {
            isSeriesDecided = true;
            seriesWinnerId = alliance1Id;
          } else if (winsA2 >= 2) {
            isSeriesDecided = true;
            seriesWinnerId = alliance2Id;
          }
        }
      } catch (e) {
        // Best-of-3 table might not have entries yet
      }
      
      // Generate matches for this series
      for (int matchNum = 1; matchNum <= matchesPerSeries; matchNum++) {
        final timeStr = formatTime(currentHour, currentMinute);
        
        // Check if this specific match has a result in bestof3 table
        bool isCompleted = false;
        int? winnerId;
        
        try {
          final bestOf3Result = await conn.execute("""
            SELECT winner_alliance_id, is_completed
            FROM tbl_championship_bestof3
            WHERE category_id = :catId 
              AND match_round = :roundNum 
              AND match_position = :matchPos
          ...
          if (bestOf3Result.rows.isNotEmpty) {
            final bo3 = bestOf3Result.rows.first.assoc();
            isCompleted = (bo3['is_completed']?.toString() ?? '0') == '1';
            winnerId = int.tryParse(bo3['winner_alliance_id']?.toString() ?? '0');
          }
        } catch (e) {
          // bestof3 table might not have entries yet
        }
        
        // Determine if Match 3 is a break (if series already decided 2-0)
        bool isBreak = false;
        if (matchNum == 3 && !isCompleted && isSeriesDecided) {
          isBreak = true;
          print("📊 Match 3 for series R${roundNumber}P${matchPosition} is BREAK (series already decided)");
        }
        
        // Create match data
        final matchData = <String, dynamic>{};
        matchData['match_id'] = seriesData['match_id'];
        matchData['round_name'] = '${seriesData['round_name']}_M$matchNum';
        matchData['match_position'] = matchPosition;
        matchData['bracket_side'] = bracketSide;
        matchData['round_number'] = roundNumber;
        matchData['alliance1_id'] = alliance1Id;
        matchData['alliance2_id'] = alliance2Id;
        matchData['match_number'] = matchNum;
        matchData['schedule_time'] = timeStr;
        
        // Set status
        if (isBreak) {
          matchData['status'] = 'break';
          matchData['winner_alliance_id'] = null;
        } else if (isCompleted) {
          matchData['status'] = 'completed';
          matchData['winner_alliance_id'] = winnerId;
        } else {
          matchData['status'] = 'pending';
          matchData['winner_alliance_id'] = null;
        }
        
        // Add alliance names and ranks
        if (alliance1 != null) {
          matchData['alliance1_name'] = '${alliance1['captain_name']} / ${alliance1['partner_name']}';
          matchData['alliance1_rank'] = '#${alliance1['alliance_rank']}';
        } else {
          matchData['alliance1_name'] = 'TBD';
          matchData['alliance1_rank'] = '?';
        }
        
        if (alliance2 != null) {
          matchData['alliance2_name'] = '${alliance2['captain_name']} / ${alliance2['partner_name']}';
          matchData['alliance2_rank'] = '#${alliance2['alliance_rank']}';
        } else {
          matchData['alliance2_name'] = 'TBD';
          matchData['alliance2_rank'] = '?';
        }
        
        // Determine display round name
        String displayRound = '';
        final side = bracketSide;
        
        if (side == 'winners') {
          if (roundNumber == 1) displayRound = 'QUARTER-FINAL';
          else if (roundNumber == 2) displayRound = 'SEMI-FINAL';
          else if (roundNumber == 3) displayRound = 'WINNER\'S FINAL';
          else displayRound = 'WINNER\'S ROUND $roundNumber';
        } else if (side == 'losers') {
          if (roundNumber == 1) displayRound = 'LOSER\'S ROUND 1';
          else if (roundNumber == 2) displayRound = 'LOSER\'S ROUND 2';
          else if (roundNumber == 3) displayRound = 'LOSER\'S ROUND 3';
          else if (roundNumber >= _getMaxLoserRound() - 1) displayRound = 'LOSER\'S FINAL';
          else displayRound = 'LOSER\'S ROUND $roundNumber';
        } else if (side == 'grand') {
          displayRound = seriesData['round_name'] == 'GF1' ? 'GRAND FINAL' : 'GRAND FINAL (RESET)';
        }
        
        matchData['display_round'] = displayRound;
        
        rows.add(matchData);
        
        // Only advance time for matches that are actually played (not breaks)
        if (!isBreak) {
          advanceTime();
        }
      }
    ...
    // Process in correct order: Winners → Losers → Grand
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    print("📊 PROCESSING BRACKET IN CORRECT ORDER:");
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    
    print("\n🏆 WINNER'S BRACKET (${winnersSeries.length} series):");
    for (int i = 0; i < winnersSeries.length; i++) {
      final series = winnersSeries[i];
      final roundNum = series['round_number'];
      final pos = series['match_position'];
      print("   ${i + 1}. Winner's Round $roundNum - Match $pos");
      await processSeries(series, "Winner's");
    }
    
    print("\n🏆 LOSER'S BRACKET (${losersSeries.length} series):");
    for (int i = 0; i < losersSeries.length; i++) {
      final series = losersSeries[i];
      final roundNum = series['round_number'];
      final pos = series['match_position'];
      print("   ${i + 1}. Loser's Round $roundNum - Match $pos");
      await processSeries(series, "Loser's");
    }
    
    print("\n🏆 GRAND FINALS (${grandSeries.length} series):");
    for (int i = 0; i < grandSeries.length; i++) {
      final series = grandSeries[i];
      final roundName = series['round_name'];
      print("   ${i + 1}. $roundName");
      await processSeries(series, "Grand");
    }
    
    print("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    print("📊 Generated ${rows.length} matches in correct order");
    print("   Winners: ${winnersSeries.length} series × $matchesPerSeries matches");
    print("   Losers: ${losersSeries.length} series × $matchesPerSeries matches");
    print("   Grand Finals: ${grandSeries.length} series × $matchesPerSeries matches");
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
          ...
          return Column(
            ...
            children: [
              ...
              ...roundMatches.map((match) {
                ...
                final isBreak = match['status'] == 'break';
                ...
                final isWinner1 = !isBreak && winnerId != null && winnerId.toString() == match['alliance1_id']?.toString();
                final isWinner2 = !isBreak && winnerId != null && winnerId.toString() == match['alliance2_id']?.toString();
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    Text(
                                      alliance1Name,
                                      style: TextStyle(
                                        color: isBreak ? Colors.white38 : (isWinner1 ? Colors.green : Colors.white),
                            ...
                            Padding(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  shape: BoxShape.circle,
                                  border: Border.all(
                                    color: isBreak ? Colors.grey : (isCompleted ? Colors.green : const Color(0xFFFFD700)),
                                    width: 2,
                                  ),
                                ),
                                child: Center(
                                  child: Text(
                                    isBreak ? '--' : (isCompleted ? 'WIN' : 'VS'),
                                    style: TextStyle(
                                      color: isBreak ? Colors.grey : (isCompleted ? Colors.green : const Color(0xFFFFD700)),
                                      fontWeight: FontWeight.bold,
                                      fontSize: 12,
